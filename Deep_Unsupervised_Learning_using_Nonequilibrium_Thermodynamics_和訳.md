...
我々の目標は、どんな複雑なデータ分布も単純で扱いやすい分布に変換する順方向（または推論）拡散過程を定義し、生成モデル分布を定義するこの拡散過程の有限時間反転を学習することである（図1参照）。まず、前方への推論拡散プロセスについて説明する。次に、逆生成拡散プロセスを学習し、確率評価に使用する方法を示す。また、逆プロセスのエントロピー境界を導出し、学習された分布がどのように任意の2番目の分布と乗算できるかを示す（例えば、画像のインペイントやノイズ除去の際に事後分布を計算するために行われるような）。
